...
void adccb(ADCDriver *adcp, adcsample_t *buffer, size_t n){
  ...
  if (adcp->state == ADC_COMPLETE) {
    ...
    sum_adc_clt=sum_adc_clt+samples[1+(i*ADC_GRP1_NUM_CHANNELS)];
    }
     adc_tps_val=sum_adc_tps/10;
     adc_clt=sum_adc_clt/10;
 ...
 static const ADCConversionGroup adcgrpcfg = {
  ...
  0,
  0,
  ADC_SMPR2_SMP_AN0(ADC_SAMPLE_239P5) | ADC_SMPR2_SMP_AN3(ADC_SAMPLE_239P5),
  0,
  ADC_SQR1_NUM_CH(ADC_GRP1_NUM_CHANNELS),
  0,
  ADC_SQR3_SQ1_N(ADC_CHANNEL_IN0) | ADC_SQR3_SQ2_N(ADC_CHANNEL_IN3)
};
